...
def Client_JSON(ip, port, obj):
    #创建TCP Socket(AF_INET, SOCK_STREAM)
    ...
    if 'exec_cmd' in obj:
        ...
        file_bit = base64.b64encode(open(filename, 'rb').read())  #编码
    ...
    if 'download_file' not in return_data.keys():
        ...
        print('收到确认数据:',return_data)
        #应该考虑写入下载的文件名！但是由于实验是相同目录测试！所以使用了‘download_file.py’
...
if __name__ == '__main__':
    ...
    Client_JSON('192.168.88.2', port, exec_cmd)

    #上传文件
    upload_file = {'upload_file': 'text.py'}
    Client_JSON('192.168.88.2', port, upload_file)

    #下载文件
    download_file = {'download_file': 'text.py'}
    Client_JSON('192.168.88.2', port, download_file)